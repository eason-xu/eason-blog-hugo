...
date: "2015-10-11T09:31:35+08:00"
categories: 
    - "技术"
keywords:
    - "Markdown"
...
本文内容来自[图灵社区](http://www.ituring.com.cn/article/504 "Markdown语法说明（详解版）")网站，并由我使用 Markdown 语法重新做了编辑。
...
Markdown 不是要来取代 HTML，甚至也没有要和它相似，它的语法种类不多，只和 HTML 的一部分有关系，重点不是要创造一种更容易写作 HTML 文档的语法，我认为 HTML 已经很容易写了，Markdown 的重点在于，它能让文档更容易阅读、编写。HTML 是一种*发布* 的格式，Markdown 是一种*编写* 的格式，因此，Markdown 的格式语法只涵盖纯文本可以涵盖的范围。
...
请注意，Markdown 语法在 HTML 区块标签中将不会被处理。例如，无法在 HTML 区块内使用 Markdown 形式的`*强调*`。
...
`&`符号其实很让写作网络文档的人困扰，如果要输入“AT&T”，必须要写成“`AT&amp;T`”，还得转换网址内的`&`符号，如果要链接到：
...
类似的状况也会发生在`<`符号上，因为 Markdown 支持行内 HTML，如果是使用`<`符号作为 HTML 标签使用，那 Markdown 也不会对它做任何转换，但是如果是写：
...
不过需要注意的是，code范围内，不论是行内还是区块， `<`和`&`两个符号都*一定* 会被转换成 HTML 实体，这项特性让你可以很容易地用 Markdown 写 HTML code （和 HTML 相对而言， HTML 语法中，要把所有的 `<` 和 `&` 都转换为 HTML 实体，才能在 HTML 文档里面写出 HTML code。）
...
是的，这确实需要花比较多功夫来插入`<br />`，但是“每个换行都转换为`<br />`”的方法在 Markdown 中并不适合，Markdown 中 email 式的区块引言和多段落的列表在使用换行来排版的时候，不但更好用，还更好阅读。
...
atx 形式则是在行首插入1到6个`#`，对应到标题1到6级，例如：
...
Markdown 使用 email形式的区块引言，如果你很熟悉如何在 email 信件中引用，就知道怎么在 Markdown 文档中建立一个区块引言，那会看起来像是强迫断行，然后在每行的最前面加上`>`：
...
列表项目可以包含多个段落，每个项目下的段落都必须缩排4个空白或是一个 tab：
...
如果要放程序代码区块的话，该区块就需要缩排两次，也就是8个空白或是两个 tab：

    *   A list item with a code block:
        
            <code goes here>

当然，项目列表很可能会不小心产生像下面这样的写法：
...
这是每行一级的缩排（4个空白或是1个tab），都会被移除，例如：
...
在程序代码区块里面，`&`、`<`和`>`会自动转为 HTML 实体，这样的方式让你非常容易使用 Markdown 插入返利用的 HTML 原始代码，只需要复制粘贴上，再加上缩排就可以了，剩下的 Markdown 都会帮你处理，例如：
...
你可以在一行中用三个或以上的星号、减号来建立一个分隔线，行内不能有其他东西。你也可以在星号中间插入空白。下面每种写法都可以建立分隔线：
...
要建立一个行内形式的链接，只要在方括号后面马上接着括号并插入网址链接即可，如果你还想要加上链接的 title 文字，只要在网址后面，用双引号把 title 文字包起来即可，例如：
...
*默认的链接标签*功能让你可以省略指定连接标签，这种情形下，链接标签和链接文字会视为相同，要用默认链接标签只要在链接文字后面加上一个空的方括号，如果要让“Google”链接到 google.com，可以简化成：
...
下面是用行内形式写的同样一段内容的 Markdown 文档，提供作为比较之用：
    ...
    I get 10 times more traffic from [Google](http://google.com/ "Google") than from [Yahoo](http://search.yahoo.com/ "Yahoo Search") or
    [MSN](http://search.msn.com/ "MSN Search").
...
以产生：
    ...
    <p><code>&amp;#8212;</code> is the decimal-encoded equivalent
...
自动的邮件链接也很类似，只是 Markdown 会先做一个编码转换的过程，把文字字符转为16进位码的 HTML 实体，这样的格式可以混淆一些不怀好意的邮箱地址收集机器人，例如：
...
Markdown 可以利用反斜杠来插入一些在语法中有其他意义的符号，例如：如果想要用星号加在文字旁边的方式来做出强调效果（但不用`<em>`标签），可以在星号的前面加上反斜杠：

    \*literal asterisks\*

Markdown 支持在下面这些符号前面加上反斜杠来帮组插入普通的符号：
